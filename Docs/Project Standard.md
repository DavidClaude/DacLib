...
- 类型：bool
- 说明：指明该对象是否可用，关键属性是否可访问，关键方法是否可调用

#### [active]

- 类型：bool
- 说明：指明该对象的功能逻辑是否处于活跃状态
...
- 说明：Awake用于开启功能；Pause用于暂停不必要的运算，但实时状态须保持不变；Continue用于基于当前状态继续运行；Reset用于关闭功能，状态须重置，回到待启用状态
- 场景：对象中包含多个可运转功能，需要依据主体状态控制其工作
- 实例：Hoxis中User在被启用时需开启心跳检测，在停用时关闭

#### [Start - Stop]

- 说明：单一功能的开启与关闭，关闭后不考虑是否可以再次开启
- 场景：对象本身即为单一可运转功能的实现
- 实例：时段计时器
